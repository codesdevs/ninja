...
xiaoQ 仅供学习参考使用，请于下载后的 24 小时内删除，本人不对使用过程中出现的任何问题负责，包括但不限于 `数据丢失` `数据泄露`。

xiaoQ 仅支持 qinglong 2.8+
...
2. 进容器内执行以下命令
   ...
   **进容器内执行以下命令**
   ...
   ```bash
   git clone https://github.com/codesdevs/ninja.git /ql/ninja
   cd /ql/ninja/backend
   pnpm install
   pm2 start
